...
cpdef partition_packed(matrix, indices):
    ...
    Nw_trg = int(ceil(Ne / <double>PACKED_SIZE))
    Nw_test = int(ceil((matrix.Ne - Ne) / <double>PACKED_SIZE))

    trg_matrix = BitPackedMatrix(np.zeros((Nf, Nw_trg), dtype=packed_type),
                                 Ne, matrix.Ni)
...
cpdef sample_packed(matrix, indices, invert=False):
    ...
    if invert:
        ...
        sample = BitPackedMatrix(np.zeros((Nf, cols), dtype=packed_type),
